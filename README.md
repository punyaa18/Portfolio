## Check out the Portfolio at https://punyaa18.github.io/Portfolio/
  
# 👋 Hi, I'm Punyaa Dixit
      
🎓 Computer Science Engineering Student |  AI & Automation Enthusiast |  Cloud & DevOps Learner
(need to update this as well)
📍 Nagpur, India  
📧 Email: punyaa184@gmail.com  
🔗 LinkedIn: https://www.linkedin.com/in/punyaa-dixit/  
💻 GitHub: https://github.com/punyaa18  

---

##  About Me

I am a **Computer Science Engineering student** with hands-on experience in **AI integration, Microsoft Power Platform, and DevOps fundamentals**.  
I enjoy building **automation-first, scalable systems** using **LLMs, cloud technologies, and low-code platforms**, and I am actively exploring real-world AI applications in enterprise and infrastructure environments.

---

## 🎓 Education

**B.Tech – Computer Science Engineering**  
G.H. Raisoni College of Engineering, Nagpur  
**2022 – 2026**  
**CGPA:** 9.63 / 10.0  
🏆 *3rd Year Department Topper (Rank 1 / 200)*

---

##  Technical Skills

**Programming & Frameworks**
- Java, Python
- Flask, Node.js

**DevOps & Cloud**
- Git, GitHub, Linux
- AWS (Fundamentals) - EC2, Elastic Beanwalk, VPS, CouldWatch,S3 , etc

**AI / ML & LLMs**
- Azure OpenAI
- Ollama (Local LLMs)
- AI Builder

**Microsoft Technologies**
- Power Automate
- Power Apps
- Microsoft Fabric
- Dataverse

**Tools**
- VS Code
- Vapi AI
- Make.com
- Copilot Studio

---

##  Experience

### Infrastructure Engineer Intern  
**TIU Consulting** | Dec 2025 – Present
- Working on DevOps and cloud infrastructure fundamentals including Git/GitHub workflows, Linux administration, and AWS services.
- Supporting CI/CD-oriented development practices and version-controlled deployments.
- Gaining exposure to infrastructure + AI by bridging backend systems with applied AI use cases.

### Software Engineering Intern  
**Perficient** | May 2025 – Aug 2025
- Designed enterprise automation workflows by integrating AI and LLMs into Microsoft Power Platform solutions.
- Automated business processes using Power Automate and Dataverse with Copilot-driven insights.
- Authored technical blogs and solution guides published on Perficient’s official platform.

### Volunteer Teacher  
**Renovatio** | Jan 2024 – May 2025
- Taught underprivileged children basic computer literacy, including Microsoft Office and internet usage.
- Designed hands-on lessons to make technology accessible while improving communication and leadership skills.

---

##  Projects

- **AI-Integrated IT Ticketing System**  
  Automated ticket classification and routing using Azure OpenAI sentiment analysis.

- **Custom Ollama Model Development**  
  Built local LLMs using Modelfiles for domain-specific and offline AI processing.

- **Microsoft Copilot Solutions (Automotive & Health)**  
  Designed Copilot-powered assistants for smart automotive insights and personalized wellness recommendations.

- **Interview Workflow Automation**  
  End-to-end interview system with scheduling, Q&A, and evaluation using Power Platform + LLMs.

- **AI Calling Assistant**  
  Automated calling and real-time conversation handling using Vapi AI and Make.com.

- **Resume Parsing (PoC – TIU Consulting)**  
  LLM-powered pipeline to extract structured resume data (skills, experience, education).

---

##  Certifications

- AI Agents & Generative AI – Microsoft, Azure OpenAI, Udemy
- Microsoft Power Platform – Power Apps, Power Automate, AI Builder
- Supervised Machine Learning – Coursera

---

##  Writing & Contributions

- **Author at Perficient Blogs**  
  Tutorials and case studies on Power Platform, LLMs, and Microsoft Fabric  
  https://blogs.perficient.com/author/pdixit/

- **GitHub Portfolio**  
  https://github.com/punyaa18

---

⭐ *Always learning, building, and exploring AI-driven automation.*

